...
func NewClient(ctx context.Context, options *resty2.Options) *resty.Client {
	...
	transport := &http.Transport{
		DisableCompression:    c.Bool(resty2.TransportDisableCompression),
		DisableKeepAlives:     c.Bool(resty2.TransportDisableKeepAlives),
		MaxIdleConnsPerHost:   c.Int(resty2.TransportMaxConnsPerHost),
		ResponseHeaderTimeout: c.Duration(resty2.TransportResponseHeaderTimeout),
		Proxy:                 http.ProxyFromEnvironment,
		DialContext:           dialer.DialContext,
		ForceAttemptHTTP2:     c.Bool(resty2.TransportForceAttemptHTTP2),
		MaxIdleConns:          c.Int(resty2.TransportMaxIdleConns),
		MaxConnsPerHost:       c.Int(resty2.TransportMaxConnsPerHost),
		IdleConnTimeout:       c.Duration(resty2.TransportIdleConnTimeout),
		TLSHandshakeTimeout:   c.Duration(resty2.TransportTLSHandshakeTimeout),
		ExpectContinueTimeout: c.Duration(resty2.TransportExpectContinueTimeout),
	}

	client.
		SetTransport(transport).
		SetTimeout(c.Duration(resty2.RequestTimeout)).
		SetRetryCount(c.Int(resty2.RetryCount)).
		SetRetryWaitTime(c.Duration(resty2.RetryWaitTime)).
		SetRetryMaxWaitTime(c.Duration(resty2.RetryMaxWaitTime)).
		SetDebug(false).
	...
	if options.Debug || c.Bool(resty2.Debug) {
...
func addTimeoutRetryCondition(client *resty.Client, options *resty2.Options) {
	...
	client.AddRetryCondition(
		func(r *resty.Response, err error) bool {
			...
			if options.RequestTimeout > 0 {
				...
				timeout = c.Duration(resty2.RequestTimeout)
			}
...
func configureHealthCheck(client *resty.Client, o *resty2.Options) {

	mc := resty2.NewRestyChecker(client, o)
