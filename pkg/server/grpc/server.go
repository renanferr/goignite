...
func Start(ctx context.Context) *grpc.Server {
	...
	if c.Bool(TlsEnabled) {

		// Load the certificates from disk
		certificate, err := tls.LoadX509KeyPair(c.String(CertFile), c.String(KeyFile))
		if err != nil {
		...
		ca, err := ioutil.ReadFile(c.String(CaFile))
		if err != nil {
		...
		s = grpc.NewServer(
			grpc.Creds(creds),
			grpc.MaxConcurrentStreams(uint32(c.Int64(MaxConcurrentStreams))),
